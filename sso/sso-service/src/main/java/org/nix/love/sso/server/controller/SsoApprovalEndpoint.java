package org.nix.love.sso.server.controller;
 ...
 * @author zhangpei
 * @version 1.0
 * @description 覆盖框架默认的授权。该煮自定义
 * @date 2019/2/5
 * @see org.springframework.security.oauth2.provider.endpoint.WhitelabelApprovalEndpoint 覆盖该类
 */
...
public class SsoApprovalEndpoint {
    ...
    private static String TEMPLATE = "<html><body><h1>OAuth Approval</h1><p>是否要授权给应用'${authorizationRequest.clientId}'</p><form id='confirmationForm' name='confirmationForm' action='${path}/oauth/authorize' method='post'><input name='user_oauth_approval' value='true' type='hidden'/>%csrf%%scopes%<label><input name='authorize' value='Authorize' type='submit'/></label></form>%denial%</body></html>";
